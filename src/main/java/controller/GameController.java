...
/**
 * Handles the operation and intelligence behind the game
 *
 * @author Timothy Baba, RuYi Marone
 */
public class GameController {
    ...
    private static boolean move(TerrainTileFX newTile) {
        ...
        if (!result) {
            ...
            newAlert.setHeaderText("You cannot carry out this move!");
            ...
            ParallelTransition transition = new ParallelTransition(newTile,
                    rotate);
    ...
    private static boolean move(TerrainTile start, TerrainTile end) {
        ...
        if (lastClicked != null) {
            RotateTransition rotate
                = new RotateTransition(Duration.millis(400));
            rotate.setToAngle(360);

            ParallelTransition transition = new ParallelTransition(lastClicked,
                    rotate);
            transition.setCycleCount(1);
    ...
    private static void attack(TerrainTileFX enemyTile) {
        ...
        if (!attacked) {
            ...
            newAlert.initOwner(CivilizationGame.getPrimaryStage());
            newAlert.setTitle("Error Message");
            newAlert.setHeaderText("You cannot carry out this attack!");
            AudioClip plonkSound = new AudioClip(CivilizationGame.getFile()
                + "wave.mp3");
            plonkSound.play();
            newAlert.showAndWait();
    ...
    public static void ai() {
        ...
        for (TerrainTile tile : bandits) {
            ...
            if (newRow >= 0 && newRow < CivilizationGame.getRowNum() && newCol
                >= 0 && newCol < CivilizationGame.getColNum()) {
    ...
    private static TerrainTile getRandomAdjacentTile(int row, int col) {
        ...
        while (!(r >= 0 && r < CivilizationGame.getRowNum() && c > 0
            && c < CivilizationGame.getColNum())) {
