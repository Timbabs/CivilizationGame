...
public class Map {
    ...
    private void populateMap() {
        TileType[][] tiles = new TileType[CivilizationGame
        .getRowNum()][CivilizationGame.getColNum()];
        ...
        for (int r = 0; r < CivilizationGame.getRowNum(); r++) {
            TileType[] row = new TileType[CivilizationGame.getColNum()];
   ...
   /* private void populateMap() {
    ...
    public static void addEnemies(Bandit bandit, int numEnemies) {
        for (int i = 0; i < numEnemies; i++) {
            int r = new Random().nextInt(CivilizationGame.getRowNum());
            int c = new Random().nextInt(CivilizationGame.getColNum());
            putSettlement("Bandit Hideout", bandit, 9, 0);
            while (!map[r][c].isEmpty()) {
                //try to get new coords
                r = new Random().nextInt(CivilizationGame.getRowNum());
                c = new Random().nextInt(CivilizationGame.getColNum());
            }
